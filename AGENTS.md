...
| 10 | `fluid publish --target <name> [--target <name>]... --env <env>` | Multi-target catalog publisher: `command-center`, `datamesh-manager`, `datahub`, `marketplace`, etc. `--target` is repeatable; accepts `NAME:endpoint` override. `--env` loads the overlay as `apply --env` does. Command Center: `is_public` only for a contract classified `public`; after each write whose contract changed, `POST /api/v1/contracts/sync` records the version with git provenance (`GIT_COMMIT`/`GIT_URL`/`GIT_BRANCH`, else the repo); `--dry-run --format json` prints the request bodies and derived lineage edges without a network call; a set-but-blank `FLUID_CC_ORG_ID` fails (`cc_organization_id_blank`). |
...
- **The chain.** Stage 1 `fluid bundle --env`; stages 2, 3, 5, 6 and 9 read `runtime/bundle.tgz`; stage 6 plans for `APPLY_MODE`, stage 7 applies `runtime/plan.json --bundle runtime/bundle.tgz` in that mode (never rewritten), with `--build-id` only when the mode is `*-and-build`. After a `dry-run` apply, stage 8 checks the bindings instead of enforcing them and stage 9 is skipped. Stage 10 is `fluid publish <contract> --env <env> --format json --target=<each of PUBLISH_TARGETS>`, its document archived as `runtime/publish-report.json`. The workspace is removed with the core `deleteDir()` in `post { cleanup }`.
...
- **Stage 1 — community (keyless, light).** Every PR, including forks. Drop `tests/providers/test_<x>_emulated*.py`, set `pytestmark = [pytest.mark.integration, pytest.mark.emulated]`, and request a fixture from `tests/_infrastructure/emulator_fixtures.py`: `moto_glue_client` (AWS), `fakesnow_patch` (Snowflake, in-process) or `fakesnow_server_target` (Snowflake over the real wire protocol — the unpatched connector against a local fakesnow server, which is what catches connector-level regressions the in-process patch cannot). `ci.yml`'s `emulated-integration` job runs `pytest -m emulated`. Locally: `pip install -e ".[dev,local,test-emulators]"` then `pytest -m emulated`.
- **Stage 2 — admin (heavy emulated).** Docker-based emulators. Set `pytestmark = [pytest.mark.integration, pytest.mark.emulated_heavy]` and use `bigquery_emulator_client` (GCP) or the LocalStack endpoint (`FLUID_LOCALSTACK_ENDPOINT`). `integration-emulated-heavy.yml` runs `pytest -m emulated_heavy` **nightly** (05:00 UTC), on `workflow_dispatch`, and on a PR carrying the `ci:integration-emulated` label — use the label to prove a change before it merges. It provisions the GCP emulator compose stack and LocalStack, then `scripts/ci/assert_lane_coverage.py` **fails the lane unless each provisioned area actually contributed a passing test** — emulator tests skip themselves when their emulator is unreachable, so without that check the lane exits 0 having proved nothing. If you add an emulator to this lane, add a matching `--require`. Locally: `pip install -e ".[dev,test-emulators-heavy]"`, `docker compose -f tests/iac/_gcp_emulator/docker-compose.yml up -d`, `FLUID_IAC_LIVE_GCP_EMULATOR=1 pytest -m emulated_heavy` (Docker required).
...
Both stages use **separate environments**, so an emulated run can never reach the live credentials. Stage 3 is admin-gated by a maintainer label **and** required-reviewer approval. Stage 2 carries no reviewer rule: it runs unattended nightly, and a reviewer gate there only queues runs until they are cancelled — which is exactly how that lane sat dead from 2026-07-13. Its PR path is gated by the `ci:integration-emulated` label, stripped on every push by `integration-label-guard.yml` so the vouch is re-earned per push; `actionlint.yml` enforces that gate via `scripts/ci/check_env_label_gate.py`, which parses the workflow YAML and fails if ANY job bound to that environment omits the label condition — a grep would pass a third job that dropped it, because the string still appears in the other two. Never give `ci.yml` cloud secrets — fork-PR safety depends on it staying secret-free.
