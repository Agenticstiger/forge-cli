...
- `fluid apply --state-backend` defaults from `FLUID_STATE_BACKEND`; an explicit empty `--state-backend ""` forces local state. A `FLUID_STATE_BACKEND` that names only a bucket (`s3://bucket`, `gcs://bucket`) keys every contract's state apart (S3 key `fluid/<safe_ident(id)>/terraform.tfstate`, GCS prefix `fluid/<safe_ident(id)>`), packaging block or not, because one CI job applies every product with it. The flag keeps its old default: that per-contract key for a contract with a `packaging` block, the shared `fluid/terraform.tfstate` otherwise. A key or prefix in either one is used as given.
