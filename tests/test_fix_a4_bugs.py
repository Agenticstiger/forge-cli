...
# ---------------------------------------------------------------------------
# Bug A5-3: _aggregate_sub_status counts ok/changed correctly
# ---------------------------------------------------------------------------


class TestAggregateSubStatus:
    """A5-3 — apply summary counts provisionDataset actions correctly."""

    def _agg(self, sub_results: List[Dict[str, Any]]) -> str:
        from fluid_build.providers.snowflake.provider_enhanced import SnowflakeProviderEnhanced

        return SnowflakeProviderEnhanced._aggregate_sub_status(sub_results)

    def test_empty_returns_skipped(self):
        assert self._agg([]) == "skipped"

    def test_error_wins(self):
        assert self._agg([{"status": "ok"}, {"status": "error"}]) == "error"

    def test_changed_returns_changed(self):
        # schema was created — should aggregate to 'changed', not 'skipped'
        assert self._agg([{"status": "changed"}, {"status": "ok"}]) == "changed"

    def test_ok_returns_ok(self):
        # schema already existed — should aggregate to 'ok', not 'skipped'
        assert self._agg([{"status": "ok"}]) == "ok"

    def test_success_returns_success(self):
        assert self._agg([{"status": "success"}]) == "success"

    def test_all_skipped_returns_skipped(self):
        assert self._agg([{"status": "skipped"}]) == "skipped"

    def test_provision_dataset_changed_increments_applied(self):
        """Regression: provisionDataset that created schema/table must count
        as applied, not skipped."""
        from fluid_build.providers.snowflake.provider_enhanced import SnowflakeProviderEnhanced

        provider = SnowflakeProviderEnhanced.__new__(SnowflakeProviderEnhanced)

        # Simulate what _handle_abstract_provision_dataset returns when
        # database+schema+table are created (all sub_results = "changed").
        sub_results = [
            {"status": "changed", "op": "sf.database.ensure", "changed": True},
            {"status": "changed", "op": "sf.schema.ensure", "changed": True},
            {"status": "changed", "op": "sf.table.ensure", "changed": True},
        ]
        overall_status = SnowflakeProviderEnhanced._aggregate_sub_status(sub_results)
        # Before the fix this returned "skipped" — now it must return "changed".
        assert overall_status == "changed"

        # Simulate the apply counter logic from provider_enhanced.apply():
        applied = 0
        mock_result = {
        ...
        if mock_result.get("status") == "changed" or (
            mock_result.get("status") == "ok" and not mock_result.get("skipped", False)
        ):
            applied += 1
        assert applied == 1, (
            "provisionDataset that created resources should count as 1 applied action, "
            f"got applied={applied}"
        )

    def test_provision_dataset_ok_increments_applied(self):
        """Regression: provisionDataset where resources already existed (ok) must
        also count as applied."""
        from fluid_build.providers.snowflake.provider_enhanced import SnowflakeProviderEnhanced

        sub_results = [
            {"status": "ok", "op": "sf.database.ensure", "changed": False},
            {"status": "ok", "op": "sf.schema.ensure", "changed": False},
        ]
        overall_status = SnowflakeProviderEnhanced._aggregate_sub_status(sub_results)
        assert overall_status == "ok"

        applied = 0
        mock_result = {"status": overall_status, "op": "provisionDataset"}
        if mock_result.get("status") == "changed" or (
            mock_result.get("status") == "ok" and not mock_result.get("skipped", False)
        ):
            applied += 1
