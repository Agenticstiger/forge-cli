...
class TestLangchainCompatibility:
    """The langchain ``BaseTool`` surface must work without forcing
    every legacy-path user to install langchain-core eagerly."""

    def test_langchain_tool_is_built_on_demand(self) -> None:
        @forge_tool(args_schema=EchoArgs)
        def echo(args):
            return args.text

        tool = FORGE_TOOL_REGISTRY["echo"]
        # Cached attribute starts unset to avoid eager imports.
        assert tool._lc_tool is None

        lc_tool = tool.langchain_tool
        # Once accessed, it's cached.
        assert tool._lc_tool is lc_tool
        # Re-access returns the same object.
        assert tool.langchain_tool is lc_tool

    def test_langchain_tool_invokes_impl_via_args_model(self) -> None:
        @forge_tool(args_schema=EchoArgs)
        def echo(args: EchoArgs):
            return f"got {args.text}"

        lc_tool = FORGE_TOOL_REGISTRY["echo"].langchain_tool
        # langchain BaseTool.invoke takes the args dict directly.
        assert lc_tool.invoke({"text": "hello"}) == "got hello"

    def test_langchain_tool_carries_args_schema(self) -> None:
        @forge_tool(args_schema=EchoArgs)
        def echo(args):
            return args.text

        lc_tool = FORGE_TOOL_REGISTRY["echo"].langchain_tool
        # Pydantic args-model is propagated for OpenAI-format tool
        # specs that langchain produces from the BaseTool.
        assert lc_tool.args_schema is EchoArgs
