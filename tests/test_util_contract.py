...
class TestGetExposeBinding:
    def test_binding_dict(self):
        b = get_expose_binding({"binding": {"platform": "aws"}})
        assert b == {"platform": "aws"}
...
# ── get_owner ───────────────────────────────────────────────────────
class TestGetOwner:
    def test_metadata_owner_shape(self):
        """Canonical 0.7.x location: metadata.owner."""
    ...
    def test_empty_metadata_owner_returns_empty_mapping(self):
        contract = {"metadata": {"owner": {}}}
        assert get_owner(contract) == {}

    def test_non_mapping_metadata_owner_returns_empty_mapping(self):
        contract = {"metadata": {"owner": "nope"}}
        assert get_owner(contract) == {}

    def test_top_level_owner_is_not_read(self):
        """Top-level ``owner`` is not a 0.7.x field — only metadata.owner."""
        contract = {"owner": {"team": "top"}}
        assert get_owner(contract) == {}
