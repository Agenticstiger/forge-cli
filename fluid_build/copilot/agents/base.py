...
class BaseStageAgent:
    ...
    def _call_once(
        ...
        *,
        ...
        # call routes through ``langchain-core`` ChatModels instead of
        # the bespoke httpx + provider-adapter machinery below.
        # Closes the parity gaps (Gemini structured output re-enabled,
        # Ollama tool_use, Anthropic prompt caching uniform) and the
        # streaming-usage gap (langchain populates ``usage_metadata``
        # on streamed and blocking calls alike).
        from fluid_build.cli.forge_copilot_lc_providers import (
            is_langchain_provider_enabled,
        )

        if is_langchain_provider_enabled(
            capability_matrix=session.capability_matrix
        ):
            return self._call_once_langchain(
                session,
                config=config,
    ...
    def _call_once_langchain(
        ...
        *,
        ...
        """Structured-output call via langchain-core ChatModels.

        World-class agent layer (opt-in via
        ``FLUID_USE_LANGCHAIN_PROVIDERS=1``). Closes the gaps the
        legacy provider abstraction couldn't address without a 2K-line
        rewrite per provider:

        * Gemini ``responseSchema`` is enabled by langchain-google-genai
          regardless of ``FLUID_GEMINI_RESPONSE_SCHEMA`` since structured
          output is the call's primary contract.
        * Streaming and blocking calls both populate
          ``AIMessage.usage_metadata`` so ``record_missing_usage`` is no
          longer the streaming default.
        * Anthropic prompt caching is on by default (system prompts are
          identical across stages within a session — high cache hit rate).

        Errors are funneled through the same
        :func:`classify_provider_error` / :class:`SchemaValidationError`
        path so :func:`retry_with_backoff` keeps its typed-error
        ...
        try:
            with AgentStatus(
                ...
                result, usage = call_structured_via_langchain(
                    config,
                    system_prompt=system_prompt,
                    user_prompt=user_prompt,
                    output_schema=output_schema,
            ...
            raise classify_provider_error(exc, provider=provider_name) from exc
        except ValidationError as exc:
            raise SchemaValidationError(
                f"Stage '{self.stage}' output failed schema validation",
                schema_name=output_schema.__name__,
        ...
        # Token-usage attribution mirrors the legacy path so the cost
        # summary's per-agent table works unchanged. The langchain
        # path closes the streaming-usage gap: even when the underlying
        # transport is SSE, ``usage_metadata`` is populated on the
        # final ``AIMessage`` so ``record_missing_usage`` is no longer
        # the streaming default.
        from fluid_build.copilot.cost import check_cost_ceiling, get_run_tracker
