...
# _mcp_app is defined here but lives in server.py (where @tool() decorators
# run at import time). We lazy-import it to break the import cycle:
# policy.py -> server.py (on first access) -> policy.py (already loaded).
def __getattr__(name: str):
    if name == "_mcp_app":
        from fluid_build.cli.mcp.server import _mcp_app as app
...
def _build_fastmcp_app(policy: McpPolicy) -> Any:
    ...
    """
    # _mcp_app lives in server.py; accessed via __getattr__ lazy import.
