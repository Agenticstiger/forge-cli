...
class FluidCLIError(_BaseCLIError):
    ...
    def format_for_user(self, console: Console) -> None:
        ...
        if self.docs_url:
            ...
            # exists for exactly this reason ("for prose that is what you want; for a
            # serialized document it is a correctness bug"), and a URL is the same
            # category. The dead `forge.fluid.dev/ref/...` links this replaced were
            # short enough to fit in 80 columns; the real ones are not, so fixing the
            # destination without this would trade a link that went nowhere for a link
            # you cannot click.
            console.print(f"\n[cyan]📖 Documentation: {self.docs_url}[/cyan]", soft_wrap=True)
